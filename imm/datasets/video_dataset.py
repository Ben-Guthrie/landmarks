...
def load_dataset(data_root, subset):
    ...
    with open(osp.join(data_root, 'subsets', 'list_training_test.txt'), 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            data = line.split()
            image_files.append(data[0])
            future_image_files[i].append(data[1])
            images_set[i].append(int(data[2]))

    assert image_files[0] =='00000.png'
